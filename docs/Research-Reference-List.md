...
- **Fix found for the UEFI-target question Task 3 left open — strip the VGA device entirely (2 Sept 2026, Task 4 live testing).** Task 3's direct-kernel-boot workaround (above) sidesteps WHPX's broken OVMF graphics by never invoking firmware graphics at all — but that also means the live environment boots BIOS-mode, so archinstall's bootloader step showed "UEFI is not detected and some options are disabled" and refused to offer `systemd-boot` (only Grub/Limine) — confirming Task 3's carried-over flag was a real blocker, not a non-issue. Root cause, isolated via a standalone test VM: OVMF still had a VGA-capable display device attached even with `-nographic` (that flag only skips opening a window; the device and OVMF's GOP init against it still exist) — so OVMF's console splitter still tried to initialize graphics on the broken WHPX path and stalled before ever producing output, even on serial. **Fix: add `-vga none`** (real OVMF UEFI firmware, `-accel whpx`, `-nographic`, no display device at all). Confirmed working: GRUB's own boot menu and its 10-second countdown rendered correctly over the serial socket — genuine UEFI firmware output, no graphics device for WHPX to fail on. This means a live session booted this way will have a real `/sys/firmware/efi` and archinstall should offer `systemd-boot` normally. **Not yet fully closed:** getting GRUB to actually boot the archiso's default entry with `console=ttyS0,115200` appended (needed so the *kernel's* output, not just firmware's, reaches serial) wasn't nailed down via scripted testing — editing the kernel line in GRUB's line editor worked fine (`e`, Ctrl+E to end of line, type the param), but neither Ctrl+X nor F10 (sent as raw bytes/escape sequences over the socket) triggered an actual boot; F10-as-`\x1b[21~"` was confirmed to be read as bare ESC (cancel) plus stray characters, since GRUB's raw serial input doesn't assemble multi-byte escape sequences into single keypresses the way a real terminal does. A human's real keypress through `vm/serial-console-raw.py` (which does proper terminal-level key handling via `ENABLE_VIRTUAL_TERMINAL_INPUT`) may succeed where the scripted byte-injection didn't — untested as of this entry. Once the boot-trigger is confirmed, `vm/launch-dev-vm.ps1` should be updated to boot via this method (OVMF + `-vga none`) instead of direct kernel boot, since it gives a genuinely correct UEFI live environment instead of a worked-around BIOS one.
