...
- **WHPX + OVMF pflash syntax is broken.** The standard `-drive if=pflash,format=raw,file=OVMF_CODE.fd` invocation fails to boot under `-accel whpx` with `WHPX: Failed to emulate MMIO access` — a long-standing, still-unfixed QEMU bug ([#513](https://gitlab.com/qemu-project/qemu/-/issues/513)). **Fix: use `-bios OVMF_CODE.fd` instead of the pflash drive syntax.** (SPEC.md's launch command already happens to use this form — confirmed correct, not luck to rely on going forward.)
