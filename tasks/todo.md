...
**Files likely touched:** `scripts/jazz-theme-set` (extended 14 Sept to also write `/var/lib/jazz/ly-theme.json`), new `scripts/jazz-ly-theme-sync` + `scripts/setup-ly-theme.sh` (14 Sept), new `configs/quickshell/Welcome.qml` + `scripts/setup-welcome.sh` (14 Sept), `scripts/setup-dock.sh` (Welcome Loader line added to its `shell.qml` heredoc), `scripts/install-jazz.sh`/`scripts/jazz-user-add` (both updated to chain `setup-ly-theme.sh`/`setup-welcome.sh`), `docs/Research-Reference-List.md` (15 Sept, USB boot-stall finding), `tasks/todo.md` Task 17 (15 Sept, draft README "Known Issues" section). **No committed setup script for Plymouth** - it was built and tested live directly on the guest (`/usr/share/plymouth/themes/jazz/`, `/etc/mkinitcpio.d/linux-lts.preset`, `/etc/os-release`, `efibootmgr`), then the splash/HOOKS portion was reverted; the surviving branding pieces (NVRAM label, os-release title, splash BMP) currently exist only as live guest state, not yet captured into a repo script - real gap if this needs to be reproducible later, deliberately not addressed tonight given the Plymouth splash itself is on hold.
**Estimated scope:** M - new-user-desktop-provisioning (item 6), its wallpaper-bootstrap follow-up (item 7), `ly` theming, the Welcome app, and three boot-branding fixes (NVRAM label/menu title/stub splash) are all done and shipped; Plymouth's own splash is attempted-and-reverted pending a real fix for the unrelated USB boot-stall; none of the surviving live-guest branding changes are yet captured into a committed, reproducible script
