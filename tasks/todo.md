...
- **Real, load-bearing finding, not yet fully live-tested (flag for Akash to confirm at the keyboard):** `nmcli device wifi connect` failed with `Error: Insufficient privileges.` when tried over this SSH session, because NetworkManager's `network-control` polkit action is `allow_active: yes` (no password needed) but only for the seat's *active* logind session — an SSH pts session doesn't count as active/local the way the real Hyprland/Quickshell session does. This should just work when Quickshell itself (running inside the real graphical session) issues the connect — that's exactly what desktop NetworkManager applets rely on everywhere — but it could not be verified remotely, the same class of gap as the keyboard-focus bug two sessions ago. **Ask Akash to click-test an actual Connect (ideally to a currently-unconnected secured network) at the real keyboard before calling this fully done.** Bluetooth needed no such flag — its control path (`bluetoothctl`/BlueZ D-Bus) worked fine over plain SSH with no privilege prompt at all, confirmed directly.
- Pairing a real Bluetooth device end-to-end also hasn't been tested (no BT peripheral was on hand this session) — the Pair button chains `pair`/`trust`/`connect`, structurally consistent with the proven-working scan/list commands, but wants a real device nearby to confirm once.

**Still pending (next slice, not yet built):** Privacy, Accessibility (real working controls required for reduced motion + UI scaling, not stubs - UI scaling specifically means systematically threading a scale token through every hardcoded pixel size across `shell.qml`/`Settings.qml`, a real refactor deserving its own dedicated pass), Developer's deeper tools.
