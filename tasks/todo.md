...
5. **`ly` doesn't recognize newly-created users until restarted - confirmed real, needs a real fix.** Found live 12 Sept 2026 via Akash's own click-test of Task 32's new "Switch to this user" button: after creating a user via Settings and logging out, `ly`'s login screen would not accept the new username - it kept authenticating back into the previously-logged-in account. Root cause confirmed directly from `ly`'s own README (`gh api repos/fairyglade/ly/contents/readme.md`, not guessed): **"Ly doesn't automatically refresh itself. To fix this you should restart Ly service... or reboot your system."** Corroborated live: `ly@tty1.service`'s `MainPID` had been running continuously for 5+ hours (`ActiveEnterTimestamp` from this morning's boot) - well before any test user existed this session, meaning it was built from a stale snapshot. **Real fix needed:** either have `jazz-user-add` trigger `systemctl restart ly@tty1.service` after a successful `useradd` (risk: this could disrupt whichever session is currently active on that TTY, since `ly` is the parent process that launched it - needs careful testing, not a blind add), or at minimum surface a clear on-screen note in Settings' Add User flow that a reboot is required before a brand-new user can actually log in. Not fixed yet - explicit Akash consent needed before testing a live `ly@tty1` restart, same risk class as this task's other boot/greeter work.
...
- [ ] A user created via Task 32's Settings UI can actually log in as themselves at `ly` without a full reboot - either a safe post-creation `ly` refresh mechanism, or (if that proves too risky) a clear on-screen note that a reboot is required first, confirmed live end-to-end
