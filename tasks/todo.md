...
- [ ] A user can change their own password via Settings UI, wired to the already-tested `jazz-user-passwd` script
- [ ] An admin can add a new real system user via the Settings UI (backend already confirmed working standalone - this checks the UI wiring)
- [ ] Removing a user requires strong explicit confirmation, explicitly asks about keeping vs deleting the home directory, and is confirmed live via the Settings UI
- [ ] Users tab lists real accounts, not a hardcoded/fake list
- [ ] A user can set a display name and it shows on the lock screen ("Welcome, <display name>") instead of the raw username
- [ ] A user can set an avatar and it shows on the lock screen and in Settings, with a sane fallback when unset
- [ ] Idle auto-lock genuinely triggers `hyprlock` after the configured timeout, confirmed live (not just config written)
- [ ] Auto-login toggle genuinely skips `ly`'s password prompt on next boot when enabled, confirmed live, with a clear on-screen warning about the tradeoff
- [ ] Admin (wheel group) access can be toggled for an existing user via the Settings UI, confirmed via `groups <user>`
...
**Files likely touched:** `configs/quickshell/Settings.qml` (new Users tab), `scripts/jazz-user-add`/`scripts/jazz-user-remove`/`scripts/jazz-user-set`/`scripts/jazz-user-passwd` (built), new `scripts/setup-hypridle.sh` (install + idle config), `scripts/jazz-theme-set` (hyprlock username label - extend to prefer display name)
**Estimated scope:** L - backend scripts done and live-tested; remaining work is the Settings UI + display name/avatar/idle-lock/auto-login extras
...
6. **New users get bare stock Hyprland, not the real JAZZ desktop - confirmed real, real fix identified but not yet built.** Found live 12 Sept 2026, same reboot test: logged in as the newly-created `jazzy` and screenshotted their session - it showed Hyprland's own generic auto-generated "Welcome to Hyprland!" tutorial popup, no JAZZ top bar/dock/Quickshell/theme at all (the same class of bug Task 21 originally fixed for the very first account, resurfacing for every account Task 32's "Add User" creates, since `jazz-user-add` only runs a bare `useradd` - none of JAZZ's own per-user provisioning). Researched the standard Linux fix (`/etc/skel` - any file placed there is automatically copied into a new user's home directory by `useradd -m`, which `jazz-user-add` already calls; this is standard `useradd` behavior, not something specific to any distro, confirmed no comparable distro needed to invent anything special here since single-user-at-install is the normal case even for Omarchy). **Real blocker found before attempting this:** JAZZ's own `scripts/setup-*.sh` files do NOT persist anywhere on the installed guest (confirmed live - `find / -iname 'setup-hyprland.sh'` returns nothing) - they're only ever pushed transiently to `/tmp` and run once per session from the *development* machine's own repo checkout. This means neither of the two candidate fixes is a trivial addition:
   - **Option A (re-run setup scripts per new user):** would first require `install-jazz.sh` to install its own `scripts/` directory persistently somewhere on the guest (e.g. `/usr/local/share/jazz/scripts/`), a real architectural change (JAZZ's install scripts becoming "installed software" the system keeps, not a one-shot provisioning tool) - `jazz-user-add` would then invoke the relevant Track B scripts (`setup-hyprland.sh`, `setup-quickshell.sh`, `setup-dock.sh`, `setup-settings.sh`, `setup-wallpaper.sh`, `setup-kvantum.sh`, `setup-theme-bundle.sh`, `setup-agent-safety.sh`) with the new username, exactly as `install-jazz.sh` already does for the first user.
   - **Option B (`/etc/skel`):** simpler in principle, but every one of JAZZ's generated configs currently bakes in absolute paths substituted at deploy time (e.g. `@@JAZZ_DATA_DIR@@` → `/home/holycowstudios/.local/share/jazz`) - copying `holycowstudios`'s own live configs into `/etc/skel` verbatim would carry those WRONG paths into every new user's copy too. Would need either a genuinely `$HOME`-relative rewrite of the placeholder-substitution scheme (a real change to `generate-theme-qml.py`/`setup-settings.sh`/etc.'s own substitution logic), or a separate "generate for /etc/skel specifically" pass that substitutes a generic path scheme instead.
   Neither option is a quick fix - this is real, scoped follow-up work, not something to rush. Not built yet.
...
- [ ] A newly-created user's first login shows the real JAZZ desktop (top bar, dock, active theme), not Hyprland's own generic stock config - blocked on deciding between Option A (persistent scripts + per-user re-provisioning) or Option B (genericized `/etc/skel`) above

**Dependencies:** Task 27b (theme tokens to source colors from), Track A/Snapper (safety net before any `mkinitcpio -P` rebuild), Task 32 (Welcome app's "add more users" nudge, once that tab exists, and the new-user-desktop-provisioning gap found via its own Add User feature)
**Files likely touched:** new `scripts/setup-plymouth.sh`, new `design/plymouth/` theme assets, `scripts/jazz-theme-set` (extend to write `ly`'s config too), new Welcome app QML + a `scripts/setup-welcome.sh`, `scripts/install-jazz.sh` (chain the new setup scripts, and possibly install `scripts/` persistently on the guest for Option A above), `scripts/jazz-user-add` (whichever option is chosen)
**Estimated scope:** L (grew from M once the new-user-desktop-provisioning gap was found) - Plymouth/initramfs work is genuinely risk-bearing (needs explicit consent + Snapper safety net before a live reboot test, same caution class as Task 27b's hyprlock test), the Welcome app itself is a straightforward, low-risk QML surface, but the new-user-provisioning fix (item 6) is a real architectural decision needing its own careful design pass
