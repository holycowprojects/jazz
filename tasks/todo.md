...
### Task 29: Jazz Files — real GUI file manager (v1, no AI) — **DONE, 15 Sept 2026**
...
**Architecture (confirmed against `docs/JAZZ-v2.md` sec 5f before writing any code):** same "Quickshell frontend + plain CLI backend" pattern as Settings.qml/Task 22, no Rust service. Backend is `scripts/jazz-files-ops` (Python 3 stdlib, one dispatcher script — `list`/`recent`/`mkdir`/`rename`/`copy`/`move`/`trash`/`list-trash`/`restore`/`empty-trash`/`delete-permanent`/`properties`/`chmod-translated`/`undo`/`mounts`/`mount`/`unmount`/`eject`, every subcommand JSON in/out). Frontend is `configs/quickshell/Files.qml`, a single file (not split into `Files/*.qml` as originally guessed in this section's "Files likely touched" — Settings.qml's own precedent of staying one file even at 2600+ lines made splitting unnecessary), loaded via `Loader { source: "Files.qml" }` in shell.qml and toggled by `qs ipc call files toggle` (same `IpcHandler` convention as Settings/Welcome/Command Centre). Every child process launches with a real argv array — no `bash -c` string-building for anything touching a user-supplied path — so a filename with spaces/`$`/backticks/quotes can never be misinterpreted by a shell.

**Real bugs found and fixed during live backend testing (15 Sept 2026), before any QML existed:**
- A manual `restore` left a stale `"trash"` entry on top of the undo log; `undo` popping it then failed (the file was already back), and because `fail()` exits before the log gets saved, that stale entry stayed on top forever, permanently blocking every undo after it. Fixed by trimming the log *before* attempting a reversal (so a failing/stale entry never blocks the stack), and by having `restore`/`empty-trash`/`delete-permanent` invalidate their own now-stale undo entries.
- `lsblk`'s singular `MOUNTPOINT` column only surfaces one of several Btrfs subvolume mounts — JAZZ's own root disk mounts `/`, `/home`, and `/.snapshots` from one physical partition (confirmed live via `findmnt -t btrfs`). Switched to `MOUNTPOINTS` (plural, JSON array) and collapsed them to one sidebar entry (preferring `/` as the browse target, others listed as `other_mounts`) instead of misleadingly showing the same disk's usage three times.

**Real bugs found and fixed during live QML testing (15 Sept 2026), all caught via Quickshell's own log (`qmllint` was tried first but exits 255 with no output even against the already-working, live Settings.qml in this environment — an environment quirk, not a useful signal, so abandoned in favor of deploy-and-check-the-real-log, same as every prior QML task in this project):**
- A missing closing paren in the Properties dialog's multi-line ternary (`text: ... ? (...) : ""`) — real parse error, `@Files.qml[871:29]: Expected token ','`.
- The Properties dialog's permission buttons were reading the *inner* Repeater's `modelData`/`index` for the who-am-I (owner/group/other), not the outer one — nested Repeaters shadow each other's context properties, so every button silently resolved to "owner" regardless of its row. Fixed by capturing the outer value on the row's own `id` (`property string who: modelData`) before the inner Repeater shadows it.
- `Quickshell.iconPath("folder", "")` (and other generic mimetype names) silently failed to resolve even though Adwaita — the real active theme per `gsettings` — genuinely ships them, because this Wayland session has no `QT_QPA` icon-theme env wired in (app-specific icons still resolve fine since many ship their own hicolor fallback). Fixed by using real, verified absolute `file://` paths into `/usr/share/icons/Adwaita/scalable/{places,mimetypes}/` (and Papirus for `application-pdf`, missing from Adwaita) instead of a named-theme lookup.
- List view's Size/Modified columns rendered with **zero gap** (`"53 B15 Sep 2026, 11:25"`) — `Size` was right-aligned to the exact edge of its own column slot, and `Row` places children edge-to-edge with no default spacing, so the next column's text started at that same pixel. Fixed with `spacing: 10` on the row plus a matching width reduction on the (flexible, elided) Name column so the columns still sum to exactly the row's width.
- Quick Look's image `Image` element gated its `visible` on `previewKind === "image"` but **not** its `source` binding — so opening Quick Look on any non-image file still attempted to decode that file as an image in the background (hidden, harmless to the user, but a real wasted decode + a log warning every time). Fixed by gating `source` the same way as `visible`.
- The breadcrumb rendered a doubled `/` (`/ / home / holycowstudios`) because the root segment's own name ("/") got an extra "/" separator appended after it. Fixed by skipping the separator specifically after the root segment.

**Real process-level lesson, worth remembering for any future live guest deploy from this Windows machine:** `pscp`-transferring an *Edited* (not newly-`Write`n) repo file straight from this Windows checkout carries real CRLF line endings (this repo's git config normalizes to LF in storage but checks out CRLF locally on Windows) — pushing `setup-dock.sh` this way made it die partway through authoring `shell.qml` (`set: -: invalid option`, `$'\r': command not found`), and pushing `setup-hyprland.sh` this way silently no-op'd (see below). No damage resulted this time only because the failing run errored out *before* ever overwriting the live file — always `sed -i "s/\r$//"` a pscp'd script on the guest before running it from here on.

**Real, deliberate (not a bug) finding about `setup-hyprland.sh`:** it only (re)writes `hyprland.lua` if the file doesn't yet exist or doesn't already contain the `JAZZ base Hyprland config` marker — so editing its heredoc (the Dolphin→Files keybind change) does **not** retroactively apply via a plain re-run on an already-provisioned machine. This is intentional: `hyprland.lua` is explicitly documented as user-hackable (`docs/Keybinds.md`), and unconditionally overwriting it on every re-run/`jazz-user-add` would silently destroy a real user's own customizations. For this dev machine (no real customizations to protect) the new keybind was applied via a direct, surgical live patch instead of a fresh regenerate.

**Icon theme note for anyone extending Files.qml later:** don't reach for `Quickshell.iconPath()` for generic (non-app) icon names in this environment — it only reliably resolves app `.desktop` icons. Use the verified-path approach `mimeIconPath()` already uses.

**Real, unrelated Home-directory cleanup found directly through testing this task, 15 Sept 2026 (Akash's direction):** the Home grid view surfaced 32 loose dev/test files (`.py`/`.log`/`.tar`/`.png` scripts and artifacts) sitting directly at `$HOME` root, cluttering the view Jazz Files exists to make usable — moved them all into a new `~/System Files` folder, leaving Home showing only real folders (`Documents`/`Downloads`/`Pictures`/`Videos`/`Music`/`Projects`/`System Files`/`ai-core`/`jazz`). This is real user-data organization on the live Yoga 6, not a repo change - nothing to commit for it, but it's exactly the kind of real-desktop-hygiene gap a working Files app is meant to surface.

**Second real bug found in the same pass (fixed before commit):** reopening the panel after files changed on disk (e.g. this exact cleanup) still showed the stale pre-change listing — the IPC `toggle()` handler only refreshed the device sidebar/app catalog on becoming visible, never re-fetched the current directory listing. Fixed by calling `refresh()` there too. That surfaced a second, more serious bug: `opProc` (the one shared `Process` every backend call reuses) has no protection against two calls racing — the new toggle-triggered `refresh()` could fire while `homeProc`'s own startup `refresh()` was still in flight, and the second call's `command`/`doneCallback` clobbered the first's mid-response, corrupting the JSON one of them received (`TypeError: Cannot read property 'length' of undefined` in the List view, confirmed live via Quickshell's log). Fixed with a small FIFO queue (`opQueue`) in front of `opProc` so concurrent calls queue instead of clobbering each other — the more general, correct fix, not just a narrow patch for this one call site.

**Acceptance criteria:**
- [x] All v1 operations (browse/copy/move/rename/trash/restore/create-folder/properties) work correctly on real files, confirmed live — backend exhaustively tested via SSH (list/mkdir/rename/copy/move/trash/restore/undo chain/properties/chmod-translated/mounts), frontend exercised via real function calls (temporary debug `Timer`s, same technique as Task 31's `contentY` trick, reverted before commit) since no click/key-simulation tool (`wtype`/`ydotool`) exists on the guest
- [x] Grid and List views both render correctly with real files (images in Grid, source/documents in List with name/type/size/modified/owner columns) — screenshot-confirmed both modes against a real `$HOME` (41 real items) and against seeded test files
- [x] Preview pane works for at least image/text/Markdown/PDF, reachable via `Space` on a selected file (Quick Look), `Escape` to close — image and real `Text.MarkdownText`-rendered Markdown (bold/bullets) screenshot-confirmed; PDF renders via `pdftoppm` (confirmed available, not yet screenshot-verified against a real PDF); the `Space`/`Escape` key bindings themselves use the exact same `Keys.on...Pressed`/`focus:` pattern already proven live elsewhere (Settings' search field, the launcher) rather than being independently key-press-tested this session
- [x] Device sidebar shows real Btrfs volume info and mount/unmount/eject work live — real usage bars confirmed screenshot (`SYSTEM_DRV` 131.1 MB/256.0 MB, `nvme0n1p5` 25.3 GB/97.7 GB); `mount`/`unmount`/`eject` call real `udisksctl` (confirmed installed, service enabled) but not exercised against a real inserted USB device this session (none attached to the Yoga 6 during this work)
- [x] Permissions are shown in translated form by default, with a working advanced/raw view — screenshot-confirmed against a real `644` file, translated pills correctly highlighted (Owner: Read & Write, Group/Other: Read), raw mode octal + real uid/gid shown
- [x] Undo works for at least move/rename/delete-to-trash, via a real transaction log (not Snapper) — full chain tested live (mkdir→rename→copy→move→trash→manual-restore→4×undo unwinds cleanly back to the exact starting state, confirmed via `ls`)
- [x] Dolphin's existing `Super+E` keybind is retired in favor of Jazz Files, or clearly repurposed — no dangling reference to the old placeholder — `fileManager` variable removed entirely from `setup-hyprland.sh`, bind now calls `qs ipc call files toggle`, confirmed live (no remaining `fileManager`/`dolphin` reference in the deployed config)

**Verification:** Live, on the Yoga 6 (real hardware) — backend fully live-tested via SSH; frontend rendering/logic live-tested via Quickshell's own real parse/runtime log plus screenshots (Grid, List, breadcrumb, sidebar+devices, context menu, Quick Look image, Quick Look Markdown, Properties+permissions, New Folder dialog, Trash view) through the *real* `install-jazz.sh`/`setup-files.sh`/`setup-dock.sh`/`setup-hyprland.sh` scripts, not just hand-patched files. Not yet click-tested by a human (no input-simulation tool available this session) — worth a real hands-on pass from Akash before calling this fully battle-tested, especially multi-select-free single-item copy/cut/paste and the context menu's screen-edge clipping at the rightmost grid column.
...
**Files touched:** `scripts/jazz-files-ops` (new), `scripts/setup-files.sh` (new — backend install, `udisks2` enable, per-user `Files.qml` deploy), `configs/quickshell/Files.qml` (new), `scripts/setup-dock.sh` (Loader line), `scripts/setup-hyprland.sh` (Dolphin keybind retired), `scripts/install-jazz.sh` / `scripts/jazz-user-add` (chain updated), `docs/Keybinds.md` (Super+E description updated)

**Estimated scope:** L — genuinely large, comparable to or bigger than Task 22's dock/launcher/settings build; matched in practice (~1300 lines of QML, ~450 lines of backend)
...
- **Avatar - built, deployed, and confirmed live on the REAL lock screen, 15 Sept 2026.** Went with the simpler JAZZ-owned scheme flagged as the likely direction earlier, not `accountsservice`: new `scripts/jazz-user-avatar` (self-service, no root) normalizes any Pillow-readable image (center-cropped to square, resized to 256x256) and writes it to `~/.face.icon` - deliberately the exact path SDDM already reads natively (confirmed via the v2 login-screen research the same night), so this work carries forward with zero migration once v2 switches the login screen. `jazz-theme-set`'s `write_hyprlock()` extended to conditionally emit a real `image { }` block (circular, accent-colored border, confirmed real hyprlock syntax via web research before writing any config - `path`/`size`/`rounding`/`border_color`/`reload_time` are all real keys) only when `~/.face.icon` exists - functional-honesty call: the block's absence *is* the fallback, no broken image reference ever gets written. Settings UI adds a "MY AVATAR" section to the Users tab (image preview with an initials-letter fallback when unset, a path field since there's no file picker before Jazz Files exists, Set/Remove buttons) - confirmed live via screenshot (scrolled the Settings content area programmatically to check it, since it renders below the fold). **Confirmed on the actual, real `hyprlock` lock screen** (not just the generated config file) - deliberately triggered a real lock this time, now that the crash-recovery procedure from the idle-lock incident earlier tonight was already known, making it a safe, bounded test: screenshot shows a real circular avatar with the accent border, correctly positioned above "Welcome to JAZZ", with the display name ("Akash Navet") rendering correctly too - confirming both this feature AND the earlier display-name-on-lock-screen item in the same shot. Recovered cleanly afterward via `hyprctl eval 'hl.clear_crashed_lockscreen()'`, test avatar removed, `hyprlock.conf` regenerated back to its no-avatar state, all processes confirmed healthy.
...
- [x] A user can set a display name and it shows on the lock screen - `jazz-theme-set`'s hyprlock label extended 14 Sept 2026 to prefer GECOS display name over raw username (falls back to username when unset). Confirmed correct for both cases (`Akash Navet`, `anve`) via the raw pipeline on 14 Sept; **confirmed a second way, on the actual real lock screen via screenshot, 15 Sept 2026** (alongside the avatar test below) - "Akash Navet" genuinely renders on `hyprlock`, not just in the generated config file.
- [x] A user can set an avatar and it shows on the lock screen and in Settings, with a sane fallback when unset - confirmed live 15 Sept 2026, both in Settings (screenshot) and on the real `hyprlock` lock screen itself (screenshot - a real circular avatar with accent border), fallback (initials in Settings, no broken image block in hyprlock.conf) confirmed by removing the test avatar and checking both surfaces again
...
**Files likely touched:** `configs/quickshell/Settings.qml` (Users tab - display name, idle lock, auto-login, avatar sections), `scripts/jazz-user-add`/`scripts/jazz-user-remove`/`scripts/jazz-user-set`(`--autologin` added)/`scripts/jazz-user-passwd`, `scripts/jazz-idle-set` (new), `scripts/setup-hypridle.sh` (new), `scripts/setup-jazz-bin.sh` (new - closes the `/usr/local/bin` reproducibility gap, later extended for `jazz-user-avatar`), `scripts/install-jazz.sh`/`scripts/jazz-user-add` (both updated to chain the above), `scripts/jazz-theme-set` (hyprlock username label extended to prefer display name; `write_hyprlock()` extended for the avatar `image` block), new `scripts/jazz-user-avatar` (15 Sept)
**Estimated scope:** L - fully done: all backend scripts, all Settings UI (users/password/add/remove/admin-toggle/display-name/idle-lock/auto-login/avatar), and both lock-screen surfaces (display name, avatar) confirmed live on the real `hyprlock` screen, not just generated config. Task 32 is complete.
